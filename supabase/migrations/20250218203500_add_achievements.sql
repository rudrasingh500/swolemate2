...
CREATE TABLE achievements (
  ...
  title TEXT NOT NULL,
  description TEXT,
  target INTEGER DEFAULT 1, -- Default target for achievements (e.g., 1 workout, 7 days)
...
ON user_achievements
FOR INSERT
WITH CHECK (true); -- Or more restrictive if needed, e.g., specific roles
...
INSERT INTO achievements (id, icon, title, description) VALUES
  (gen_random_uuid(), '🎉', 'First Workout', 'Complete your first workout session.'),
  (gen_random_uuid(), '🔥', 'Week Streak', 'Complete workouts for 7 consecutive days.'),
  (gen_random_uuid(), '🏆', 'Form Master', 'Achieve a perfect form score in 5 different exercises.');
...
BEGIN
  ...
  VALUES (new.id, new.email, new.raw_user_meta_data->>'username', new.raw_user_meta_data->>'full_name', new.raw_user_meta_data->>'avatar_url', NOW());
